...
// assigned to the Google Business Profile Agent?" against BossAgent's
...
export const GOOGLE_BUSINESS_PROFILE_AGENT_ID = "google-business-profile-agent";

/** True if `decision` was auto-assigned (or human-confirmed) to this agent. */
export function isGoogleBusinessProfileAssignment(decision: RoutingDecision): boolean {
  return decision.status === "assigned" && decision.assignedAgentId === GOOGLE_BUSINESS_PROFILE_AGENT_ID;
}
