...
// assigned to the Graphic Design Agent?" against BossAgent's public
...
export const GRAPHIC_DESIGN_AGENT_ID = "graphic-design-agent";

/** True if `decision` was auto-assigned (or human-confirmed) to this agent. */
export function isGraphicDesignAssignment(decision: RoutingDecision): boolean {
  return decision.status === "assigned" && decision.assignedAgentId === GRAPHIC_DESIGN_AGENT_ID;
}
