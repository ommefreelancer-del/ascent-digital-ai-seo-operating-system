...
// assigned to the Campaign Tracking Agent?" against BossAgent's public
...
export const CAMPAIGN_TRACKING_AGENT_ID = "campaign-tracking-agent";

/** True if `decision` was auto-assigned (or human-confirmed) to this agent. */
export function isCampaignTrackingAssignment(decision: RoutingDecision): boolean {
  return decision.status === "assigned" && decision.assignedAgentId === CAMPAIGN_TRACKING_AGENT_ID;
}
