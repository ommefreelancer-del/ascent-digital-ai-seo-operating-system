...
// assigned to the Competitor Intelligence Agent?" against BossAgent's
...
export const COMPETITOR_INTELLIGENCE_AGENT_ID = "competitor-intelligence-agent";

/** True if `decision` was auto-assigned (or human-confirmed) to this agent. */
export function isCompetitorIntelligenceAssignment(decision: RoutingDecision): boolean {
  return decision.status === "assigned" && decision.assignedAgentId === COMPETITOR_INTELLIGENCE_AGENT_ID;
}
