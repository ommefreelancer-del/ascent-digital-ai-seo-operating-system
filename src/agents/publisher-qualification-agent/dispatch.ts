...
// assigned to the Publisher Qualification Agent?" against BossAgent's
...
export const PUBLISHER_QUALIFICATION_AGENT_ID = "publisher-qualification-agent";

/** True if `decision` was auto-assigned (or human-confirmed) to this agent. */
export function isPublisherQualificationAssignment(decision: RoutingDecision): boolean {
  return decision.status === "assigned" && decision.assignedAgentId === PUBLISHER_QUALIFICATION_AGENT_ID;
}
