...
import { exchangeCodeForTokens, saveConnection } from "@/server/google-sheets";
...
export async function GET(request: Request) {
  ...
  const cookieState = cookieStore.get("google_sheets_oauth_state")?.value;
  ...
  try {
    ...
    await logActivity(session.user.id, "integrations", "Connected Google Sheets");
