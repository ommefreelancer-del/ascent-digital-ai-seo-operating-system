...
import { exchangeCodeForTokens, saveConnection } from "@/server/google-business-profile";
...
export async function GET(request: Request) {
  ...
  const cookieState = cookieStore.get("gbp_oauth_state")?.value;
  ...
  try {
    ...
    await logActivity(session.user.id, "integrations", "Connected Google Business Profile");
