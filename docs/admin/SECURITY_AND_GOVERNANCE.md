...
- **Confirmed encrypted as of this pass (direct call-site verification):** `GitHubConnection`'s access token, Google Search Console's tokens, and WordPress's app password/access token fields (both providers).
- **Not independently call-site-verified in this pass:** the shared Google service connection tokens (`GoogleServiceConnection` — Business Profile, Sheets, Analytics, Drive, Gmail). A schema comment claims the same AES-256-GCM mechanism, but this pass did not confirm an `encryptSecret(`/`decryptSecret(` call site for this model the way it did for the three connection types above — see `technical/SECURITY_ARCHITECTURE.md` and `technical/DOCUMENTATION_CONFLICTS.md`. Do not represent this as confirmed to a client or in an audit until it is call-site-verified.
